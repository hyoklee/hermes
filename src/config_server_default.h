...
const char* kServerDefaultConfigStr =
    ...
    "  ram:\n"
    "    # The mount point of each device. RAM should be the empty string. For "
    "block\n"
    "    # devices, this is the directory where Hermes will create buffering "
    "files. For\n"
    ...
    "    capacity: 50MB\n"
    "\n"
    "    # The size of the smallest available buffer in KiB. In general this "
    "should be\n"
    "    # the page size of your system for byte addressable storage, and the "
    "block size\n"
    "    # of the storage device for block addressable storage.\n"
    "    block_size: 4KB\n"
    "\n"
    "    # The number of blocks (the size of which is chosen in "
    "block_sizes_kb) that each\n"
    "    # device should contain for each slab (controlled by num_slabs). This "
    "allows for\n"
    "    # precise control of the distibution of buffer sizes.\n"
    "    slab_sizes: [ 4KB, 16KB, 64KB, 1MB ]\n"
    "\n"
    "    # The maximum theoretical bandwidth (as advertised by the "
    "manufacturer) in\n"
    ...
    "    latency: 15us\n"
    "\n"
    "    # For each device, indicate \'1\' if it is shared among nodes (e.g., "
    "burst\n"
    "    # buffers), or \'0\' if it is per node (e.g., local NVMe).\n"
    "    is_shared_device: false\n"
    "\n"
    "    # For each device, the minimum and maximum percent capacity threshold "
    "at which\n"
    "    # the BufferOrganizer will trigger. Decreasing the maximum thresholds "
    "will cause\n"
    "    # the BufferOrganizer to move data to lower devices, making more room "
    "in faster\n"
    "    # devices (ideal for write-heavy workloads). Conversely, increasing "
    "the minimum\n"
    "    # threshold will cause data to be moved from slower devices into "
    "faster devices\n"
    "    # (ideal for read-heavy workloads). For example, a maximum capacity "
    "threshold of\n"
    "    # 0.8 would have the effect of always keeping 20% of the device\'s "
    "space free for\n"
    "    # incoming writes. Conversely, a minimum capacity threshold of 0.3 "
    "would ensure\n"
    ...
    "    borg_capacity_thresh: [ 0.0, 1.0 ]\n"
    "\n"
    "# Define the maximum amount of memory Hermes can use for non-buffering "
    "tasks.\n"
    "# This includes metadata management and memory allocations.\n"
    "# This memory will not be preallocated, so if you don\'t know, 0 "
    "indicates\n"
    ...
    "buffer_organizer:\n"
    "  # The number of threads used in the background organization of internal "
    "Hermes buffers.\n"
    "  num_threads: 1\n"
    "\n"
    ...
    "\n"
    "  # If true (1) the RoundRobin placement policy algorithm will split each "
    "Blob\n"
    ...
    "  is_mpi: false\n"
    "\n"
    "# The shared memory prefix for the hermes shared memory segment. A user "
    "name\n"
    "# will be automatically appended.\n"
    "shmem_name: \"/hermes_shm_\"\n"
    "\n"
    "# The interval in milliseconds at which to update the global system "
    "view.\n"
