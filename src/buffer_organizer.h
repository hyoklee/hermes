...
typedef hipc::pair<_BorgIoThreadQueue, BorgIoThreadQueueInfo> BorgIoThreadQueue;
...
class BorgIoThreadManager {
 public:
  hipc::vector<BorgIoThreadQueue> *queues_; /**< Shared-memory request queues */
  std::atomic<bool> kill_requested_; /**< Kill flushing threads eventually */

 public:
  /** Constructor */
  BorgIoThreadManager() { kill_requested_.store(false); }

  /** Indicate that no more modifications will take place to blobs */
  void Join() { kill_requested_.store(true); }
...
class BufferOrganizer {
 public:
  ...
  RPC_TYPE *rpc_;
  hipc::vector<BorgIoThreadQueue> *queues_; /** Async tasks for BORG. */
