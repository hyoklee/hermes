...
struct ConstructTask : public CreateTaskStateTask {
  ...
  HSHM_ALWAYS_INLINE explicit ConstructTask(
      ...
      const TaskStateId &bkt_mdm_id, const TaskStateId &blob_mdm_id)
...
struct RegisterOpTask : public Task, TaskFlags<TF_SRL_SYM | TF_REPLICA> {
  ...
  /** SHM default constructor */
  HSHM_ALWAYS_INLINE explicit RegisterOpTask(hipc::Allocator *alloc)
      : Task(alloc) {}
  ...
  /** Destructor */
  ~RegisterOpTask() { HSHM_DESTROY_AR(op_graph_); }
...
struct RegisterDataTask : public Task, TaskFlags<TF_LOCAL> {
  ...
  /** SHM default constructor */
  HSHM_ALWAYS_INLINE explicit RegisterDataTask(hipc::Allocator *alloc)
      : Task(alloc) {}
...
struct RunOpTask : public Task, TaskFlags<TF_LOCAL | TF_REPLICA> {
  /** SHM default constructor */
  HSHM_ALWAYS_INLINE explicit RunOpTask(hipc::Allocator *alloc) : Task(alloc) {}

  /** Emplace constructor */
  HSHM_ALWAYS_INLINE explicit RunOpTask(hipc::Allocator *alloc,
                                        const TaskNode &task_node,
                                        const TaskStateId &state_id)
      : Task(alloc) {
    // Initialize task
  ...
  /** Duplicate message */
  void Dup(hipc::Allocator *alloc, RunOpTask &other) { task_dup(other); }
