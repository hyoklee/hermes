...
class Client : public TaskLibClient {
 public:
  ...
  void SetBucketMdmRoot(const DomainId &domain_id, const TaskStateId &blob_mdm,
                        const TaskStateId &stager_mdm,
                        const TaskStateId &op_mdm) {
  ...
  void AsyncGetOrCreateBlobIdConstruct(GetOrCreateBlobIdTask *task,
                                       const TaskNode &task_node, TagId tag_id,
                                       const hshm::charbuf &blob_name) {
    u32 hash = HashBlobName(tag_id, blob_name);
    HRUN_CLIENT->ConstructTask<GetOrCreateBlobIdTask>(
        task, task_node, DomainId::GetNode(HASH_TO_NODE_ID(hash)), id_, tag_id,
        blob_name);
  }
   ...
   * */
  void AsyncPutBlobConstruct(PutBlobTask *task, const TaskNode &task_node,
                             TagId tag_id, const hshm::charbuf &blob_name,
                             const BlobId &blob_id, size_t blob_off,
                             size_t blob_size, const hipc::Pointer &blob,
                             float score, u32 flags, Context ctx = Context(),
                             u32 task_flags = TASK_FIRE_AND_FORGET |
                                              TASK_DATA_OWNER |
                                              TASK_LOW_LATENCY) {
    HRUN_CLIENT->ConstructTask<PutBlobTask>(
        task, task_node, DomainId::GetNode(blob_id.node_id_), id_, tag_id,
        blob_name, blob_id, blob_off, blob_size, blob, score, flags, ctx,
        task_flags);
  }
  HRUN_TASK_NODE_PUSH_ROOT(PutBlob);

  /** Get a blob's data */
  void AsyncGetBlobConstruct(GetBlobTask *task, const TaskNode &task_node,
                             const TagId &tag_id,
                             const hshm::charbuf &blob_name,
                             const BlobId &blob_id, size_t off,
                             ssize_t data_size, hipc::Pointer &data,
                             Context ctx = Context(), u32 flags = 0) {
    // HILOG(kDebug, "Beginning GET (task_node={})", task_node);
    HRUN_CLIENT->ConstructTask<GetBlobTask>(
        task, task_node, DomainId::GetNode(blob_id.node_id_), id_, tag_id,
        blob_name, blob_id, off, data_size, data, ctx, flags);
  }
  size_t GetBlobRoot(const TagId &tag_id, const BlobId &blob_id, size_t off,
                     ssize_t data_size, hipc::Pointer &data,
                     Context ctx = Context(), u32 flags = 0) {
    LPointer<hrunpq::TypedPushTask<GetBlobTask>> push_task = AsyncGetBlobRoot(
        tag_id, hshm::charbuf(""), blob_id, off, data_size, data, ctx, flags);
    push_task->Wait();
   ...
   * */
  void AsyncReorganizeBlobConstruct(
      ReorganizeBlobTask *task, const TaskNode &task_node, const TagId &tag_id,
      const hshm::charbuf &blob_name, const BlobId &blob_id, float score,
      bool user_score, const Context &ctx = Context(),
    ...
    HRUN_CLIENT->ConstructTask<ReorganizeBlobTask>(
        task, task_node, DomainId::GetNode(blob_id.node_id_), id_, tag_id,
        blob_name, blob_id, score, user_score, ctx, task_flags);
  }
   ...
   * */
  void AsyncTagBlobConstruct(TagBlobTask *task, const TaskNode &task_node,
                             const TagId &tag_id, const BlobId &blob_id,
                             const TagId &tag) {
    HRUN_CLIENT->ConstructTask<TagBlobTask>(task, task_node,
                                            DomainId::GetNode(blob_id.node_id_),
                                            id_, tag_id, blob_id, tag);
  }
  void TagBlobRoot(const TagId &tag_id, const BlobId &blob_id,
                   const TagId &tag) {
   ...
   * */
  void AsyncBlobHasTagConstruct(BlobHasTagTask *task, const TaskNode &task_node,
                                const TagId &tag_id, const BlobId &blob_id,
                                const TagId &tag) {
  ...
  bool BlobHasTagRoot(const TagId &tag_id, const BlobId &blob_id,
                      const TagId &tag) {
   ...
   * Get \a blob_name BLOB from \a bkt_id bucket
   * */
  void AsyncGetBlobIdConstruct(GetBlobIdTask *task, const TaskNode &task_node,
                               const TagId &tag_id,
                               const hshm::charbuf &blob_name) {
    u32 hash = HashBlobName(tag_id, blob_name);
  ...
  BlobId GetBlobIdRoot(const TagId &tag_id, const hshm::charbuf &blob_name) {
  ...
  void AsyncGetBlobNameConstruct(GetBlobNameTask *task,
                                 const TaskNode &task_node, const TagId &tag_id,
                                 const BlobId &blob_id) {
    HRUN_CLIENT->ConstructTask<GetBlobNameTask>(
        task, task_node, DomainId::GetNode(blob_id.node_id_), id_, tag_id,
        blob_id);
  }
  std::string GetBlobNameRoot(const TagId &tag_id, const BlobId &blob_id) {
  ...
  size_t GetBlobSizeRoot(const TagId &tag_id, const hshm::charbuf &blob_name,
                         const BlobId &blob_id) {
  ...
  float GetBlobScoreRoot(const TagId &tag_id, const BlobId &blob_id) {
  ...
  std::vector<BufferInfo> GetBlobBuffersRoot(const TagId &tag_id,
                                             const BlobId &blob_id) {
    LPointer<hrunpq::TypedPushTask<GetBlobBuffersTask>> push_task =
        AsyncGetBlobBuffersRoot(tag_id, blob_id);
    push_task->Wait();
   ...
   * in \a bkt_id bucket.
   * */
  void AsyncRenameBlobConstruct(RenameBlobTask *task, const TaskNode &task_node,
                                const TagId &tag_id, const BlobId &blob_id,
  ...
  void RenameBlobRoot(const TagId &tag_id, const BlobId &blob_id,
                      const hshm::charbuf &new_blob_name) {
  ...
  void TruncateBlobRoot(const TagId &tag_id, const BlobId &blob_id,
                        size_t new_size) {
  ...
  void DestroyBlobRoot(const TagId &tag_id, const BlobId &blob_id,
                       bool update_size = true) {
  ...
  void AsyncFlushDataConstruct(FlushDataTask *task, const TaskNode &task_node,
                               size_t period_ms) {
    HRUN_CLIENT->ConstructTask<FlushDataTask>(task, task_node, id_, period_ms);
  }
  ...
  void AsyncPollBlobMetadataConstruct(PollBlobMetadataTask *task,
                                      const TaskNode &task_node) {
    HRUN_CLIENT->ConstructTask<PollBlobMetadataTask>(task, task_node, id_);
  }
  std::vector<BlobInfo> PollBlobMetadataRoot() {
    ...
    std::vector<BlobInfo> blob_mdms = task->DeserializeBlobMetadata();
    HRUN_CLIENT->DelTask(push_task);
    return blob_mdms;
  }
  ...
  void AsyncPollTargetMetadataConstruct(PollTargetMetadataTask *task,
                                        const TaskNode &task_node) {
    HRUN_CLIENT->ConstructTask<PollTargetMetadataTask>(task, task_node, id_);
  }
  std::vector<TargetStats> PollTargetMetadataRoot() {
    ...
    std::vector<TargetStats> target_mdms = task->DeserializeTargetMetadata();
    HRUN_CLIENT->DelTask(push_task);
    return target_mdms;
  }
