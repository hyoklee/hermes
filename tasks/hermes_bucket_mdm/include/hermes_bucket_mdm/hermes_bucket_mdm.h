...
class Client : public TaskLibClient {
 public:
  ...
  /** Sets the BLOB MDM */
  void AsyncSetBlobMdmConstruct(SetBlobMdmTask *task, const TaskNode &task_node,
                                const DomainId &domain_id,
                                const TaskStateId &blob_mdm,
                                const TaskStateId &stager_mdm) {
    HRUN_CLIENT->ConstructTask<SetBlobMdmTask>(task, task_node, domain_id, id_,
                                               blob_mdm, stager_mdm);
  }
  void SetBlobMdmRoot(const DomainId &domain_id, const TaskStateId &blob_mdm,
                      const TaskStateId &stager_mdm) {
  ...
  void AsyncUpdateSizeConstruct(UpdateSizeTask *task, const TaskNode &task_node,
                                TagId tag_id, ssize_t update, int mode) {
    HRUN_CLIENT->ConstructTask<UpdateSizeTask>(
        task, task_node, DomainId::GetNode(tag_id.node_id_), id_, tag_id,
        update, mode);
  }
  ...
  void AsyncAppendBlobSchemaConstruct(AppendBlobSchemaTask *task,
                                      ...
                                      size_t data_size, size_t page_size) {
  ...
  /** Append data to the bucket (fire & forget) */
  HSHM_ALWAYS_INLINE
  void AsyncAppendBlobConstruct(AppendBlobTask *task, const TaskNode &task_node,
                                TagId tag_id, size_t data_size,
                                const hipc::Pointer &data, size_t page_size,
                                float score, u32 node_id, const Context &ctx) {
    HRUN_CLIENT->ConstructTask<AppendBlobTask>(
        task, task_node, DomainId::GetLocal(), id_, tag_id, data_size, data,
        page_size, score, node_id, ctx);
  }
  HSHM_ALWAYS_INLINE
  void AppendBlobRoot(TagId tag_id, size_t data_size, const hipc::Pointer &data,
                      size_t page_size, float score, u32 node_id,
                      const Context &ctx) {
  ...
  void AsyncGetOrCreateTagConstruct(GetOrCreateTagTask *task,
    ...
    HRUN_CLIENT->ConstructTask<GetOrCreateTagTask>(task, task_node, id_,
                                                   tag_name, blob_owner, traits,
                                                   backend_size, flags, ctx);
  }
  HSHM_ALWAYS_INLINE
  TagId GetOrCreateTagRoot(const hshm::charbuf &tag_name, bool blob_owner,
                           const std::vector<TraitId> &traits,
                           size_t backend_size, u32 flags,
                           const Context &ctx = Context()) {
  ...
  /** Get tag ID */
  void AsyncGetTagIdConstruct(GetTagIdTask *task, const TaskNode &task_node,
                              const hshm::charbuf &tag_name) {
  ...
  /** Get tag name */
  void AsyncGetTagNameConstruct(GetTagNameTask *task, const TaskNode &task_node,
                                const TagId &tag_id) {
  ...
  hshm::string GetTagNameRoot(const TagId &tag_id) {
    ...
    hshm::string tag_name =
        hshm::to_charbuf<hipc::string>(*task->tag_name_.get());
    HRUN_CLIENT->DelTask(push_task);
    return tag_name;
  }
  ...
  /** Rename tag */
  void AsyncRenameTagConstruct(RenameTagTask *task, const TaskNode &task_node,
                               const TagId &tag_id,
  ...
  /** Destroy tag */
  void AsyncDestroyTagConstruct(DestroyTagTask *task, const TaskNode &task_node,
                                const TagId &tag_id) {
    u32 hash = tag_id.hash_;
  ...
  /** Add a blob to a tag */
  void AsyncTagAddBlobConstruct(TagAddBlobTask *task, const TaskNode &task_node,
  ...
  void AsyncGetSizeConstruct(GetSizeTask *task, const TaskNode &task_node,
                             const TagId &tag_id) {
    u32 hash = tag_id.hash_;
  ...
  void AsyncPollTagMetadataConstruct(PollTagMetadataTask *task,
                                     const TaskNode &task_node) {
    HRUN_CLIENT->ConstructTask<PollTagMetadataTask>(task, task_node, id_);
  }
  std::vector<TagInfo> PollTagMetadataRoot() {
    ...
    std::vector<TagInfo> target_mdms = task->DeserializeTagMetadata();
    HRUN_CLIENT->DelTask(push_task);
    return target_mdms;
  }
