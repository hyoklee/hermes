...
#define NUMBER_SERIAL(type) return std::to_string(num_.type);
...
class SizeType : public Formattable {
 public:
  ...
  static const size_t BYTES = 1, KB = (1ul << 10), MB = (1ul << 20),
                      GB = (1ul << 30), TB = (1ul << 40);
  ...
  std::string ToString() { return std::to_string(num_) + unit_to_str(unit_); }
};
