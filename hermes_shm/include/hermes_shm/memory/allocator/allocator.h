...
struct AllocatorHeader {
  ...
  void Configure(allocator_id_t allocator_id, AllocatorType type,
...
class Allocator {
 ...
 public:
   ...
   * */
  virtual void shm_deserialize(char *buffer, size_t buffer_size) = 0;
  ...
  HSHM_ALWAYS_INLINE bool Reallocate(POINTER_T &p, size_t new_size) {
    ...
    auto new_p = ReallocateOffsetNoNullCheck(p.ToOffsetPointer(), new_size);
  ...
  HSHM_ALWAYS_INLINE T *AllocatePtr(size_t size, POINTER_T &p,
                                    size_t alignment = 0) {
   ...
   * */
  template <typename T, typename POINTER_T = Pointer>
  HSHM_ALWAYS_INLINE T *ClearAllocatePtr(size_t size, POINTER_T &p,
                                         size_t alignment = 0) {
    p = Allocate<POINTER_T>(size, alignment);
    if (p.IsNull()) {
      return nullptr;
   ...
   * @return A process-specific pointer
   * */
  template <typename T, typename POINTER_T = Pointer, typename... Args>
  HSHM_ALWAYS_INLINE T *AllocateConstructObjs(size_t count, POINTER_T &p,
                                              Args &&...args) {
   ...
   * */
  template <typename T, typename POINTER_T = Pointer, typename... Args>
  HSHM_ALWAYS_INLINE T *ReallocateConstructObjs(POINTER_T &p, size_t old_count,
                                                size_t new_count,
                                                Args &&...args) {
    T *ptr = ReallocatePtr<T>(p, new_count * sizeof(T));
  ...
  HSHM_ALWAYS_INLINE static void ConstructObjs(T *ptr, size_t old_count,
                                               size_t new_count,
                                               Args &&...args) {
    if (ptr == nullptr) {
      return;
  ...
  HSHM_ALWAYS_INLINE POINTER_T Convert(const T *ptr) {
    if (ptr == nullptr) {
      return POINTER_T::GetNull();
    }
    return POINTER_T(GetId(), reinterpret_cast<size_t>(ptr) -
