...
struct bitfield {
  ...
  inline bool OrBits(T mask) const { return bits_ & mask; }
...
#define INHERIT_BITFIELD_OPS(BITFIELD_VAR, MASK_T)                            \
  inline void SetBits(MASK_T mask) { BITFIELD_VAR.SetBits(mask); }            \
  inline void UnsetBits(MASK_T mask) { BITFIELD_VAR.UnsetBits(mask); }        \
  inline bool OrBits(MASK_T mask) const { return BITFIELD_VAR.OrBits(mask); } \
  inline void Clear() { BITFIELD_VAR.Clear(); }                               \
