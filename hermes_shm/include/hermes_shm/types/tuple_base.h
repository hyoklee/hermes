...
struct TupleBase {
  ...
  /** General Constructor. */
  template <typename... CArgs>
  explicit TupleBase(Args &&...args) : recur_(std::forward<Args>(args)...) {}

  /** Move constructor */
  TupleBase(TupleBase &&other) noexcept : recur_(std::move(other.recur_)) {}
  ...
  /** Copy constructor */
  TupleBase(const TupleBase &other) : recur_(other.recur_) {}
...
struct MakeConstexpr {
  ...
  constexpr static T Get() { return val_; }
};
