...
struct nonatomic {
  ...
  HSHM_ALWAYS_INLINE nonatomic operator++(int) { return atomic(x + 1); }
...
struct atomic {
  ...
  /** Atomic post-increment operator */
  HSHM_ALWAYS_INLINE atomic operator++(int) { return atomic(x + 1); }
  ...
  /** Atomic add operator */
  HSHM_ALWAYS_INLINE atomic operator+(T count) const { return x + count; }
