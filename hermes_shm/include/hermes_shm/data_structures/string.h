...
class string : public ShmContainer {
 ...
 public:
  ...
  void shm_init_main(TYPED_HEADER *header, Allocator *alloc, const char *text) {
  ...
  void shm_init_main(TYPED_HEADER *header, Allocator *alloc,
    ...
    shm_init_main(header, alloc, length);
    memcpy(text_, text1.data(), text1.size());
    memcpy(text_ + text1.size(), text2.data(), text2.size());
    text_[length] = 0;
  }
  ...
  void shm_init_main(TYPED_HEADER *header, Allocator *alloc, size_t length) {
    shm_init_allocator(alloc);
    shm_init_header(header);
    text_ = alloc_->template AllocatePtr<char>(length + 1, header_->text_);
  ...
  void shm_deserialize_main() {
    text_ = alloc_->template Convert<char>(header_->text_);
  ...
  char *c_str() const { return text_; }

  /** Get a constant reference to the C-style string */
  char *data() const { return text_; }

  /** Get a mutable reference to the C-style string */
  char *data_mutable() { return text_; }
