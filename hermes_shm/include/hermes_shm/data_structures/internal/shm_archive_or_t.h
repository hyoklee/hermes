...
class _ShmHeaderOrT_T {
 ...
 public:
  /** Default constructor */
  _ShmHeaderOrT_T() { Allocator::ConstructObj<T>(internal_ref(nullptr)); }

  /** Construct + store object (C++ argpack) */
  template <typename... Args>
  explicit _ShmHeaderOrT_T(Allocator *alloc, Args &&...args) {
    Allocator::ConstructObj<T>(internal_ref(alloc),
  ...
  void PiecewiseInit(Allocator *alloc, ArgPackT &&args) {
    hermes_shm::PassArgPack::Call(
        MergeArgPacks::Merge(make_argpack(internal_ref(alloc)),
                             std::forward<ArgPackT>(args)),
        [](auto &&...Args) constexpr {
          Allocator::ConstructObj<T>(std::forward<decltype(Args)>(Args)...);
        });
  ...
  _ShmHeaderOrT_T(_ShmHeaderOrT_T &&other) noexcept {
    Allocator::ConstructObj<_ShmHeaderOrT_T>(obj_,
                                             std::move(other.internal_ref()));
  }
  ...
  _ShmHeaderOrT_T(const _ShmHeaderOrT_T &other) {
    Allocator::ConstructObj<_ShmHeaderOrT_T>(obj_, other);
  }
...
class ShmHeaderOrT {
 public:
  ...
  /** Construct + store object */
  template <typename... Args>
  explicit ShmHeaderOrT(Args &&...args) : obj_(std::forward<Args>(args)...) {}
  ...
  /** Returns a reference to the internal object */
  T_Ar internal_ref(Allocator *alloc) { return obj_.internal_ref(alloc); }

  /** Returns a reference to the internal object */
  T_Ar internal_ref(Allocator *alloc) const { return obj_.internal_ref(alloc); }

  /** Shm destructor */
  void shm_destroy(Allocator *alloc) { obj_.shm_destroy(alloc); }

  /** Move constructor */
  ShmHeaderOrT(ShmHeaderOrT &&other) noexcept : obj_(std::move(other.obj_)) {}
  ...
  /** Copy constructor */
  ShmHeaderOrT(const ShmHeaderOrT &other) : obj_(other.obj_) {}

  /** Copy assignment operator */
  ShmHeaderOrT &operator=(const ShmHeaderOrT &other) { obj_ = other.obj_; }
};
