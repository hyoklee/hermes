...
class ShmSmartPtr : public ShmSmartPointer {
 ...
 public:
  ...
  const T* operator->() const { return get_const(); }

  /** Destroy the data allocated by this pointer */
  void shm_destroy() { obj_.shm_destroy(); }
};
...
#define SHM_SERIALIZE_WRAPPER(AR_TYPE)                                       \
  ...
  void shm_serialize(hipc::TypedAtomicPointer<TYPE_UNWRAP(AR_TYPE)>& type)   \
      const {                                                                \
    obj_.shm_serialize(type);                                                \
  }                                                                          \
