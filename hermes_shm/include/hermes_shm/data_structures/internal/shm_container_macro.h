...
#define SHM_CONTAINER_TEMPLATE(CLASS_NAME, TYPED_CLASS, TYPED_HEADER)          \
 public:                                                                       \
  ...
  hipc::Allocator                                                              \
      *alloc_; /**< hipc::Allocator used for this data structure */            \
 ...
 public:                                                                       \
  ...
  void shm_init(hipc::Allocator *alloc, Args &&...args) {                      \
    shm_destroy(false);                                                        \
    shm_init_main(hipc::typed_nullptr<TYPE_UNWRAP(TYPED_HEADER)>(), alloc,     \
                  std::forward<Args>(args)...);                                \
  }                                                                            \
                                                                               \
  /** Constructor. Initialize an already-allocated header. */                  \
  template <typename... Args>                                                  \
  void shm_init(TYPE_UNWRAP(TYPED_HEADER) & header, hipc::Allocator * alloc,   \
                Args && ...args) {                                             \
    shm_destroy(false);                                                        \
   ...
   * */                                                                        \
  template <typename... Args>                                                  \
  void shm_init_header(TYPE_UNWRAP(TYPED_HEADER) * header, Args && ...args) {  \
    if (IsValid()) {                                                           \
      header_->SetBits(SHM_CONTAINER_DATA_VALID);                              \
    } else if (header == nullptr) {                                            \
      hipc::Pointer p;                                                         \
      header_ =                                                                \
          alloc_->template AllocateConstructObjs<TYPE_UNWRAP(TYPED_HEADER)>(   \
  ...
  void shm_serialize(hipc::TypedPointer<TYPE_UNWRAP(TYPED_CLASS)> &ar) const { \
    ar = alloc_->template Convert<TYPE_UNWRAP(TYPED_HEADER), hipc::Pointer>(   \
        header_);                                                              \
    shm_serialize_main();                                                      \
  }                                                                            \
                                                                               \
  /** Serialize into an AtomicPointer */                                       \
  void shm_serialize(hipc::TypedAtomicPointer<TYPE_UNWRAP(TYPED_CLASS)> &ar)   \
      const {                                                                  \
    ar = alloc_->template Convert<TYPE_UNWRAP(TYPED_HEADER),                   \
                                  hipc::AtomicPointer>(header_);               \
    shm_serialize_main();                                                      \
  }                                                                            \
                                                                               \
  /** Override << operators */                                                 \
  ...
  bool shm_deserialize(                                                        \
      const hipc::TypedPointer<TYPE_UNWRAP(TYPED_CLASS)> &ar) {                \
  ...
  bool shm_deserialize(hipc::Allocator *alloc,                                 \
    ...
    return shm_deserialize(                                                    \
        alloc, alloc->Convert<TYPE_UNWRAP(TYPED_HEADER), hipc::OffsetPointer>( \
                   header_ptr));                                               \
  }                                                                            \
                                                                               \
  ...
  TYPE_UNWRAP(CLASS_NAME)(TYPE_UNWRAP(CLASS_NAME) && other) noexcept {         \
    shm_weak_move(hipc::typed_nullptr<TYPE_UNWRAP(TYPED_HEADER)>(),            \
                  hipc::typed_nullptr<hipc::Allocator>(), other);              \
  }                                                                            \
                                                                               \
  /** Move assignment operator */                                              \
  TYPE_UNWRAP(CLASS_NAME) &operator=(TYPE_UNWRAP(CLASS_NAME) &&                \
                                     other) noexcept {                         \
    if (this != &other) {                                                      \
      shm_weak_move(hipc::typed_nullptr<TYPE_UNWRAP(TYPED_HEADER)>(),          \
                    hipc::typed_nullptr<hipc::Allocator>(), other);            \
  ...
  TYPE_UNWRAP(CLASS_NAME) &operator=(const TYPE_UNWRAP(CLASS_NAME) & other) {  \
    if (this != &other) {                                                      \
      shm_strong_copy(hipc::typed_nullptr<TYPE_UNWRAP(TYPED_HEADER)>(),        \
                      hipc::typed_nullptr<hipc::Allocator>(), other);          \
    }                                                                          \
    return *this;                                                              \
  }                                                                            \
