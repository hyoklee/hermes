...
struct vector_iterator_templ {
 public:
  ...
  HSHM_ALWAYS_INLINE T &operator*() { return vec_->data_ar()[i_].get_ref(); }
...
class vector : public ShmContainer {
 ...
 public:
  ...
  /** Check if null */
  HSHM_ALWAYS_INLINE bool IsNull() const { return vec_ptr_.IsNull(); }
  ...
  HSHM_ALWAYS_INLINE T &front() { return (*this)[0]; }

  /** Get last element of vector */
  HSHM_ALWAYS_INLINE T &back() { return (*this)[size() - 1]; }
  ...
  void emplace_back(Args &&...args) {
    ...
    HSHM_MAKE_AR(vec[length_], GetAllocator(), std::forward<Args>(args)...)
    ++length_;
  }
  ...
  void emplace(iterator_t pos, Args &&...args) {
    ...
    shift_right(pos);
    HSHM_MAKE_AR(vec[pos.i_], GetAllocator(), std::forward<Args>(args)...)
    ++length_;
  }
  ...
  HSHM_ALWAYS_INLINE void clear() { erase(begin(), end()); }
 ...
 private:
   ...
   * */
  template <typename... Args>
  ShmArchive<T> *grow_vector(ShmArchive<T> *vec, size_t max_length, bool resize,
                             Args &&...args) {
    // Grow vector by 25%
    if (max_length == 0) {
    ...
    if constexpr (std::is_pod<T>() && !IS_SHM_ARCHIVEABLE(T)) {
      ...
      new_vec = GetAllocator()->template ReallocateObjs<ShmArchive<T>>(
          vec_ptr_, max_length);
    } else {
      // Use std::move for unpredictable objects
      Pointer new_p;
      new_vec = GetAllocator()->template AllocateObjs<ShmArchive<T>>(max_length,
                                                                     new_p);
      for (size_t i = 0; i < length_; ++i) {
        T &old_entry = (*this)[i];
        HSHM_MAKE_AR(new_vec[i], GetAllocator(), std::move(old_entry))
      }
      if (!vec_ptr_.IsNull()) {
        GetAllocator()->Free(vec_ptr_);
      }
    ...
    if (resize) {
      for (size_t i = length_; i < max_length; ++i) {
        HSHM_MAKE_AR(new_vec[i], GetAllocator(), std::forward<Args>(args)...)
      }
    }
 ...
 public:
  ...
  riterator_t rbegin() { return riterator_t(this, size<off64_t>() - 1); }

  /** End of the reverse iterator */
  riterator_t rend() { return citerator_t(this, (off64_t)-1); }
