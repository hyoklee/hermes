...
class mpsc_queue_templ : public ShmContainer {
 ...
 public:
  ...
  explicit mpsc_queue_templ(Allocator *alloc, size_t depth = 1024) {
  ...
  explicit mpsc_queue_templ(Allocator *alloc, const mpsc_queue_templ &other) {
    shm_init_container(alloc);
    SetNull();
  ...
  mpsc_queue_templ(Allocator *alloc, mpsc_queue_templ &&other) noexcept {
    shm_init_container(alloc);
 ...
 private:
  ...
  HSHM_ALWAYS_INLINE void _emplace(_qtok_t tail, Args &&...args) {
    ...
    (*queue_).replace(iter, hshm::PiecewiseConstruct(), make_argpack(),
