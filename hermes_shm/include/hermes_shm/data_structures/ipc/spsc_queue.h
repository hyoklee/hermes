...
class spsc_queue_templ : public ShmContainer {
 ...
 public:
  ...
  explicit spsc_queue_templ(Allocator *alloc, const spsc_queue_templ &other) {
    shm_init_container(alloc);
    SetNull();
  ...
  spsc_queue_templ(Allocator *alloc, spsc_queue_templ &&other) noexcept {
    shm_init_container(alloc);
 ...
 public:
  ...
  qtok_t pop(T &val) { return _pop(val); }
