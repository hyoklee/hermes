...
class smart_ptr_base {
 ...
 public:
  ...
  HSHM_ALWAYS_INLINE T *get() { return obj_; }

  /** Gets a pointer to the internal object */
  HSHM_ALWAYS_INLINE const T *get() const { return obj_; }

  /** Dereference operator */
  HSHM_ALWAYS_INLINE T &operator*() { return *get(); }

  /** Constant Dereference operator */
  HSHM_ALWAYS_INLINE const T &operator*() const { return *get(); }
  ...
  HSHM_ALWAYS_INLINE smart_ptr_base &operator=(
      smart_ptr_base &&other) noexcept {
    if (this != &other) {
      shm_strong_move(std::forward<smart_ptr_base>(other));
    }
    return *this;
   ...
   * Hash function
   * ===================================*/
  HSHM_ALWAYS_INLINE size_t hash() const { return std::hash<T>{}(*obj_); }
};
...
struct hash<hshm::ipc::mptr<T>> {
  size_t operator()(const hshm::ipc::mptr<T> &obj) const { return obj.hash(); }
};

/** Hash function for uptr */
template <typename T>
struct hash<hshm::ipc::uptr<T>> {
  size_t operator()(const hshm::ipc::uptr<T> &obj) const { return obj.hash(); }
};
