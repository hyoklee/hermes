 ...
 public:
  ...
  void ServerInit(RpcContext *rpc) {
    ...
    HILOG(kInfo, "Serving {} (i.e., {}) with {} RPC threads as node id {}",
          rpc_server_name, addr, rpc->num_threads_, rpc->node_id_);
    ClientInit(rpc);
  }
  ...
  void ClientInit(RpcContext *rpc) {
    ...
    HILOG(
        kInfo, "This client is on node {} (i.e., {}, proto: {})", rpc->node_id_,
        rpc->GetHostNameFromNodeId(DomainId::GetNode(rpc->node_id_)), protocol);
  }
  ...
  /** Stop this daemon */
  void StopThisDaemon() { StopDaemon(rpc_->node_id_); }
  ...
  std::string GetServerName(u32 node_id) {
    std::string ip_address =
        rpc_->GetIpAddressFromNodeId(DomainId::GetNode(node_id));
  ...
  thallium::async_response AsyncCall(u32 node_id, const std::string &func_name,
                                     Args &&...args) {
  ...
  /** I/O transfers */
  template <typename RetT, bool ASYNC, typename... Args>
  RetT IoCall(i32 node_id, const std::string &func_name, IoType type,
              char *data, size_t size, Args &&...args) {
    HILOG(kDebug, "Calling {} {} -> {}", func_name, rpc_->node_id_, node_id)
    try {
      std::string server_name = GetServerName(node_id);
      tl::bulk_mode flag;
  ...
  template <typename RetT, typename... Args>
  RetT SyncIoCall(i32 node_id, const std::string &func_name, IoType type,
                  char *data, size_t size, Args &&...args) {
    return IoCall<RetT, false>(node_id, func_name, type, data, size,
                               std::forward<Args>(args)...);
  }
  ...
  size_t IoCallServer(const tl::request &req, const tl::bulk &bulk, IoType type,
    ...
    try {
      ...
      HELOG(
          kFatal, "(node {}) Failed to perform bulk I/O thallium: {} (type={})",
