...
struct DataTransferBase {
  ...
  bool operator==(const DataTransferBase &other) const {
    return flags_.bits_ == other.flags_.bits_ && data_ == other.data_ &&
           data_size_ == other.data_size_ && node_id_ == other.node_id_;
  }
};
...
class BinaryOutputArchive {
 ...
 public:
  /** Default constructor */
  BinaryOutputArchive(const DomainId &node_id) : node_id_(node_id), ar_(ss_) {}
...
class BinaryInputArchive {
 ...
 public:
  ...
  HSHM_ALWAYS_INLINE
  BinaryInputArchive &Deserialize(u32 replica) { return *this; }
};
