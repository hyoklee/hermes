...
struct WorkEntry {
  ...
  /** Check if null */
  [[nodiscard]] HSHM_ALWAYS_INLINE bool IsNull() const {
...
struct hash<hrun::WorkEntry> {
  HSHM_ALWAYS_INLINE
  std::size_t operator()(const hrun::WorkEntry &key) const {
...
class Worker {
 public:
  ...
  u32 retries_; /**< The number of times to repeat the internal run loop before
                   sleeping */
 ...
 public:
  ...
  Worker(u32 id, int cpu_id, ABT_xstream &xstream) {
    ...
    /* int ret = ABT_thread_create_on_xstream(xstream,
                                           [](void *args) {
    ((Worker*)args)->Loop(); }, this, ABT_THREAD_ATTR_NULL, &tl_thread_); if
    (ret != ABT_SUCCESS) { HELOG(kFatal, "Couldn't spawn worker");
    }*/
  }
  ...
  void _PollQueues() {
    ...
    while (!poll_queues_.pop(work_queue).IsNull()) {
      for (const WorkEntry &entry : work_queue) {
        // HILOG(kDebug, "Scheduled queue {} (lane {})", entry.queue_->id_,
        // entry.lane_);
  ...
  void _RelinquishQueues() {
    ...
    while (!poll_queues_.pop(work_queue).IsNull()) {
      for (auto &entry : work_queue) {
        work_queue_.erase(
            std::find(work_queue_.begin(), work_queue_.end(), entry));
      }
    }
  }

  /** Check if worker is still stealing queues */
  bool IsRelinquishingQueues() { return relinquish_queues_.size() > 0; }
  ...
  void EnableContinuousPolling() { flags_.SetBits(WORKER_CONTINUOUS_POLLING); }
  ...
  bool IsContinuousPolling() { return flags_.Any(WORKER_CONTINUOUS_POLLING); }
    ...
    while (orchestrator->IsAlive()) {
      try {
        ...
        HELOG(kError, "(node {}) Worker {} caught an error: {}",
              HRUN_CLIENT->node_id_, id_, e.what());
  ...
  void PollGrouped(WorkEntry &work_entry, bool flushing) {
    ...
    while (!lane->peek(entry, off).IsNull()) {
      ...
      if (!exec) {
        bool was_end =
            HRUN_TASK_REGISTRY->task_states_.find(task->task_state_) ==
...
// #define HERMES_REMOTE_DEBUG
      ...
      if (!task->IsRunDisabled() && group_avail && should_run) {
        ...
        if (is_remote) {
          ...
          HRUN_REMOTE_QUEUE->DisperseLocal(task, exec, work_entry.queue_,
                                           work_entry.group_);
          task->SetDisableRun();
          ...
          if (!task->IsStarted()) {
            ...
            rctx.jmp_.fctx =
                bctx::make_fcontext((char *)rctx.stack_ptr_ + stack_size_,
  ...
  // HSHM_ALWAYS_INLINE
  bool CheckTaskGroup(Task *task, TaskState *exec, u32 lane_id, TaskNode node,
    ...
    if (ret == TASK_UNORDERED || task->IsUnordered()) {
      //      HILOG(kDebug, "(node {}) Task {} is unordered, so count remains 0
      //      worker={}",
    ...
    if (it == group_map_.end()) {
      ...
      //      (worker={})",
      //            HRUN_CLIENT->node_id_, std::hash<hshm::charbuf>{}(group_),
  ...
  HSHM_ALWAYS_INLINE
  void RemoveTaskGroup(Task *task, TaskState *exec, u32 lane_id,
                       const bool &is_remote) {
    ...
    int ret = exec->GetGroup(task->method_, task, group_);
    if (ret == TASK_UNORDERED || task->IsUnordered()) {
      //      HILOG(kDebug, "(node {}) Decreasing depth of group remains 0
      //      (task_node={} worker={})",
      //            HRUN_CLIENT->node_id_, task->task_node_, id_);
      return;
    }
    ...
    if (node_cmp.node_depth_ == 0) {
      HELOG(kFatal,
            "(node {}) Group {} depth is already 0 (task_node={} worker={})",
            HRUN_CLIENT->node_id_, std::hash<hshm::charbuf>{}(group_),
            task->task_node_, id_);
    }
    node_cmp.node_depth_ -= 1;
    //    HILOG(kDebug, "(node {}) Decreasing depth of to {} (task_node={}
    //    worker={})",
    //          HRUN_CLIENT->node_id_, node_cmp.node_depth_, task->task_node_,
    //          id_);
    if (node_cmp.node_depth_ == 0) {
    ...
    for (std::unique_ptr<Worker> &worker : HRUN_WORK_ORCHESTRATOR->workers_) {
      for (WorkEntry &work_entry : worker->work_queue_) {
        ...
        while (!lane->peek(entry, off).IsNull()) {
          ...
          HILOG(kInfo,
                "(node {}, worker {}) Task {} state {}, method {}, is remote: "
                "{}, long_running: {}",
                HRUN_CLIENT->node_id_, worker->id_, task->task_node_,
                exec->name_, task->method_, is_remote, task->IsLongRunning());
          off += 1;
        }
      }
    }
