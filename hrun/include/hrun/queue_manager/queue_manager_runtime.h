...
#define HRUN_QM_RUNTIME (&HRUN_RUNTIME->queue_manager_)
...
class QueueManagerRuntime : public QueueManager {
 ...
 public:
  ...
  void ServerInit(hipc::Allocator *alloc, u32 node_id, ServerConfig *config,
                  QueueManagerShm &shm) {
    config_ = config;
    Init(node_id);
    ...
    std::vector<PriorityInfo> queue_info{
        ...
        {TaskPrio::kLowLatency, qm.max_lanes_, qm.max_lanes_, qm.queue_depth_,
         QUEUE_LOW_LATENCY},
        {TaskPrio::kLongRunningTether, qm.max_lanes_, qm.max_lanes_,
         qm.queue_depth_, QUEUE_LONG_RUNNING | QUEUE_TETHERED,
