...
struct PushTask : public Task, TaskFlags<TF_LOCAL> {
  ...
  /** SHM default constructor */
  HSHM_ALWAYS_INLINE explicit PushTask(hipc::Allocator *alloc) : Task(alloc) {}

  /** Emplace constructor */
  HSHM_ALWAYS_INLINE explicit PushTask(
      hipc::Allocator *alloc, const TaskNode &task_node,
      const DomainId &domain_id, const TaskStateId &state_id,
      std::vector<DomainId> &domain_ids, Task *orig_task, TaskState *exec,
      u32 exec_method, std::vector<DataTransfer> &xfer)
      : Task(alloc) {
    // Initialize task
    ...
    // TASK_REMOTE_DEBUG_MARK | TASK_FIRE_AND_FORGET);
    // task_flags_.SetBits(TASK_LOW_LATENCY | TASK_COROUTINE |
    // TASK_REMOTE_DEBUG_MARK | TASK_FIRE_AND_FORGET);
    task_flags_.SetBits(TASK_LOW_LATENCY | TASK_REMOTE_DEBUG_MARK |
...
struct DupTask : public Task, TaskFlags<TF_LOCAL> {
  ...
  /** SHM default constructor */
  HSHM_ALWAYS_INLINE explicit DupTask(hipc::Allocator *alloc) : Task(alloc) {}

  /** Emplace constructor */
  HSHM_ALWAYS_INLINE explicit DupTask(hipc::Allocator *alloc,
                                      const TaskNode &task_node,
    ...
    method_ = Method::kDup;
    task_flags_.SetBits(TASK_LOW_LATENCY | TASK_REMOTE_DEBUG_MARK |
                        TASK_FIRE_AND_FORGET | TASK_COROUTINE);
